$folder = Read-Host "Where should files save"
$names = Get-ChildItem . -Filter "*.mp4" -Name -File
...
Start-Process "ffmpeg" -ArgumentList "-i", "`"$($name)`"", "-b:v", $cVideoBitRate[$vidBitRate-1], "-b:a", $cAudioBitRate[$audBitRate-1], "-r", $cFrames[$frameRate - 1], "-ar", $cSampleRate[$sampl-1], "-s", $cResolution[$resol-1], "`"$($folder)/$($name)`"" -NoNewWindow -Wait
}
