...
$resFile = Read-Host "What file should name"

Start-Process "ffmpeg" -ArgumentList "-i", "file1.mp3", "-i", "file2.mp3", "-filter_complex", "[0:a]atrim=0:duration=$($duration1)[a0];[1:a]atrim=0:duration=$($duration2)[a1];[a0][a1]amerge=inputs=2[aout]", "-map", "[aout]", "$($resFile).mp3" -NoNewWindow -Wait
